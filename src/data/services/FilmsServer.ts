...
class FilmsServer {
	...
	getAllFilms<T>(page: number, language: string, gender: string, sortBy: string, year: string): Promise<T> {
		return this.httpClient.get(`discover/movie?${API_KEY}&language=${language}&sort_by=${sortBy}&include_adult=false&include_video=false&page=${page}&year=${year}&${gender && `&with_genres=${gender}`}& with_watch_monetization_types=flatrate`);
	}
